# 从 Sangfor EasyConnect 迁移到 zju-connect

> 一次完整的 CQUPT 校园 VPN 故障排查、根因分析、方案对比与迁移实录。
>
> 记录时间：2026-04-18
> 环境：Windows 11 Home，CQUPT（重庆邮电大学）校园网
...
关键点：**EasyConnect UI 不直接连 VPN 服务器，它先调用本地 ECAgent 的 HTTPS API**。这一步看似多余，实际是为了把复杂的协议逻辑封装到守护进程里。但也正是这一步引入了我们的核心问题。
...
*本文档基于一次真实故障排查整理而成，目标是把零散的诊断过程、网络知识点、方案权衡沉淀下来，帮助后来者少走弯路。*
